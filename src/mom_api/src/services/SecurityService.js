 ...
 * - [PMP-242] Support multiple social networks
 *
 * Changes in 1.5 (FOUNDERSHARE (AKA PMP) - AUTHENTICATION AND SOCIAL MEDIA INTEGRATION ENHANCEMENT - 2)
 * - [PMP-237] Added switchRole method
 * - [PMP-237] Updated authenticateWithSessionToken method to include role to req.user object if exists in SessionToken
 *
 * @version 1.5
 * @author TCSASSEMBLER
 */
...
function authenticateWithSocialNetwork(socialNetwork, accessToken, callback) {
    ...
    var getProfile = function (error, response, body) {
        ...
        if (response.statusCode === 200) {
            ...
            var errorMessage;
            if (socialNetwork === Const.SocialNetwork.TWITTER && body.errors) {
                errorMessage = "Twitter error: " + body.errors[0].message;
            } else {
                errorMessage = "Cannot get profile: " + JSON.stringify(body);
            }
    ...
    async.waterfall([
        ...
        function (result, cb) {
            ...
            if (socialNetwork === Const.SocialNetwork.FACEBOOK) {
                ...
                //shouldn't happen
                return cb(new Error('"' + socialNetwork + '" social network not supported'));
            }
            profile.linkedSocialNetworks = [
                {
...
/**
 * Generate the session token to be returned to client in case of login request
 * @param {String} token the token to switch role
 * @param {String} role The User Role to embed with session token
 * @param {function(Error, String)} cb the callback function with arguments
 * - the error
 * - the session token
 */
function switchRole(token, role, cb) {
    var error = validate(
        {token: token, role: role},
        {token: "ShortString", role: "UserRole"});
    if (error) {
        return cb(error);
    }
    ...
    async.waterfall([
        function (cb) {
            ...
            sessionToken.role = role;
            sessionToken.save(cb);
        }
    ], cb.wrap(function (sessionToken) {
        cb(null, sessionToken.token);
    }))
}
...
function authenticateWithSessionToken(token, callback) {
    ...
    var expirationDate, sessionToken;
    async.waterfall([
        function (cb) {
            SessionToken.findOne({token: token}, cb);
        }, function (sessionToken, cb) {
        ...
        }, function (sToken, count, cb) {
            sessionToken = sToken;
            expirationDate = sessionToken.expirationDate;
            helper.ensureExists(User, sessionToken.userId, cb);
        }
    ], function (err, user) {
        user = helper.mapUser(user);
        // If session token has role set then set it in user
        if(sessionToken && sessionToken.role) {
            user.loggedInAs = sessionToken.role;
        }
...
_.extend(module.exports, {
    ...
    getSocialNetworkProfile: logging.createWrapper(getSocialNetworkProfile, {
        input: ["socialNetwork", "accessToken"],
        output: ["profile"],
        signature: "SecurityService#getSocialNetworkProfile"
    }),
    ...
    switchRole: logging.createWrapper(switchRole, {
        input: ["token", "role"],
        output: ["token"],
        logOutput: false,
        signature: "SecurityService#switchRole"
    }),
