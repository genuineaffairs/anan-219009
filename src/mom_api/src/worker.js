 ...
 *
 * Changes in version 1.5 (Authentication and Social Media Integration enhancement):
 * - [PMP-183] fix issue checkExpiredUserEmail (wrong condition on verifyEmailText)
 *
 * Changes in version 1.6 (FOUNDERSHARE (AKA PMP) - AUTHENTICATION AND SOCIAL MEDIA INTEGRATION ENHANCEMENT - 2)
 * - [PMP-202] Added sendEmailVerificationReminder
 *
 * @author TCSASSEMBLER
 * @version 1.6
 *
...
function checkExpiredUserEmail() {
    ...
    async.waterfall([
        function (cb) {
            var criteria = {
                $and: [
                    {verifyEmailExpirationDate: { $lte: date } },
                    {verifyEmailText: {$ne: 'verified'} },
                    {verifyEmailText: {$ne: null} }
                ]
            };
            User.find(criteria, cb);
        }, function (users, cb) {
...
/**
 * Send reminder email to un-verified accounts.
 */

function sendEmailVerificationReminder() {
    var date = new Date().getTime() - config.USER_EMAIL_EXPIRATION_DAYS * 24 * 60 * 60 * 1000;
    date = new Date(date);
    async.waterfall([
        function (cb) {
            var criteria = {
                $and: [
                    {verifyEmailExpirationDate: {$gt: date}},
                    {verifyEmailText: {$ne: 'verified'}},
                    {verifyEmailText: {$ne: null}},
                    {isStopVerificationReminder: {$ne: true}}
                ]
            };
            User.find(criteria, cb);
        }, function (users, cb) {
            async.forEach(users, function (user, cb) {
                ...
                var values = {
                    verifyUrl: config.DESKTOP_APP_URL + "#/VerifyEmail/" + user.id + "/" + user.verifyEmailText,
                    stopVerifyUrl: config.DESKTOP_APP_URL + "#/StopVerify/" + user.id + "/" + user.verifyEmailText,
                    firstName: user.firstName
                };

                NotificationService.sendEmail(user.email, "email-verification-reminder", values, cb);
            }, cb)
        }
    ], function (err) {
        if (err) {
            logging.logError("sendEmailVerificationReminder", err);
        }
        setTimeout(sendEmailVerificationReminder, config.EMAIL_VERIFICATION_REMINDER_INTERVAL);
    });
}
